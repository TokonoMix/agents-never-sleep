...
1. Copy the matching config into that platform's hooks location and **replace `<SKILL_DIR>`** with the
   ...
   Each command is wired as `bash <SKILL_DIR>/hooks/enforce.sh <platform> <event>` — invoked
   explicitly via `bash`, so `enforce.sh` does not need to be executable (no `chmod +x` step; a
   pip/wheel install's packaged copy loses the +x bit anyway).
2. The dispatcher is inert unless `UE_UNATTENDED=1` (or `CLAUDE_UNATTENDED=1`) is set — so once wired,
...
3. For `never-stop` to work, the platform must see the run-incomplete sentinel — export
