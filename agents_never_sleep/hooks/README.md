...
1. Merge `hooks/settings-snippet.json` into the `"hooks"` block of `~/.claude/settings.json`
   ...
   skill install. Each wired command is `bash /ABSOLUTE/PATH/TO/agents-never-sleep/hooks/<script>.sh`
   — invoked explicitly via `bash`, so the hook scripts do not need to be executable (no
   `chmod +x` step; a pip/wheel install's packaged copy loses the +x bit anyway).
2. Restart Claude Code so the hooks load.
3. Verify they are inert normally (no `CLAUDE_UNATTENDED`) and active under an unattended run.
